...
def train_stage1(model, train_loader, val_loader, criterion, optimizer, scheduler,
                 ...
                 log_target_min=-9.0, log_target_max=-0.3, run_id=''):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model.state_dict(), f'params/last_model_{run_id}.pth')
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), f'params/best_model_{run_id}.pth')
        else:
            patience_counter += 1
    ...
    plot_loss_curve(train_losses, val_losses, filename=f'figures/loss_curve_stage1_{run_id}.png')
    return model
...
def train_stage2(model, train_loader, val_loader, criterion, device,
    ...
    plot_loss_curve(train_losses, val_losses, filename=f'figures/loss_curve_stage2_{run_id}.png')
    return model
